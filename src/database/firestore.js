...
const ARCHIVE_USERS_FIELD = 'archiveUsers'
...
export const getUserRooms = (currentUserId, userId) => {
  ...
  return getDocuments(
    query(roomsRef, where(USERS_PATH, '==', [currentUserId]))
  )
}
...
export const updateRoomArchiveUsers = (roomId, currentUserId, action) => {
	const arrayUpdate =
		action === 'add' ? arrayUnion(currentUserId) : arrayRemove(currentUserId)

	return updateRoom(roomId, { [ARCHIVE_USERS_FIELD]: arrayUpdate })
}

export const updateRoomBlockedUsers = (roomId, currentUserId, action) => {
	const arrayUpdate =
		action === 'add' ? arrayUnion(currentUserId) : arrayRemove(currentUserId)

	return updateRoom(roomId, { [BLOCKED_USERS_FIELD]: arrayUpdate })
}
