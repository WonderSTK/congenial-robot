<template>
  <div class="bg-base-100 rounded-box shadow-lg px-4 py-2 mb-4">
    <div class="flex flex-wrap gap-2 items-center justify-end w-full my-4">
      <!-- Dropdown sort by -->
      <div class="flex flex-wrap items-center gap-2 mr-auto">
        <span>Sort by:</span>
        <div class="dropdown">
          <label tabindex="0">
            <select v-model="sortField" name="type" class="hidden select badge gap-2 h-10 min-h-0 rounded-full badge-accent" required>
              <option value="selected_date" selected></option>
              <option value="added_date"></option>
            </select>
            <span ref="span" @click="isDropdownOpen = !isDropdownOpen" class="select badge gap-2 h-10 min-h-0 rounded-full text-accent-content bg-accent">{{ sortField.replace('_', ' ').toUpperCase() }}</span>
          </label>
          <ul tabindex="0" class="dropdown-content menu p-2 shadow rounded-box w-52 mt-2 text-accent-content bg-accent" :class="isDropdownOpen ? '' : ' hidden'">
            <li :class="sortField === 'selected_date' ? 'hidden' : ''">
              <a @click="sortField = 'selected_date'; isDropdownOpen = false;">
                Selected Date
              </a>
            </li>
            <li :class="sortField === 'added_date' ? 'hidden' : ''">
              <a @click="sortField = 'added_date'; isDropdownOpen = false;">
                Added Date
              </a>
            </li>
          </ul>
      ...
      <div class="flex items-center">
        <input v-model="sortDirection" type="checkbox" class="toggle" />
        <span class="mx-2">{{ sortDirection ? 'DESC' : 'ASC' }}</span>
      </div>
    </div>

    <!-- Legend for colors -->
    <div class="flex flex-wrap items-center w-full gap-2 my-4 select-none">
      <div class="w-28 h-7 text-center whitespace-nowrap bg-base-100 border border-error rounded-box"><p>{{ '<' }} 1 day</p></div>
      <div class="w-28 h-7 text-center whitespace-nowrap bg-base-100 border border-warning rounded-box"><p>{{ '<' }} 3 days</p></div>
      <div class="w-28 h-7 text-center whitespace-nowrap bg-base-100 border border-error bg-danger rounded-box"><p>Unread</p></div>
    </div>
  </div>

  <ul class="space-y-4 pb-2">
    <div
      v-for="news in savedNewsListSorted"
      :key="news.id"
      class="ease flex gap-4 items-center justify-between bg-base-100 rounded-box p-3 hover:bg-base-200"
      :class="getStyle(news)"
      @click="showReadManagement(news)"
    >
      <a :href="news.link" target="_blank" class="flex flex-wrap gap-4 w-full group">
        ...
        <div class="flex-col flex-[2]">
          <div class="flex gap-2 items-center">
            ...
            <span class="text-sm text-accent lg:max-w-[15rem] truncate">{{ news.readLater[0].my_author || news.author || 'NO AUTHOR' }}</span>
            <ReadLaterButton :news="{ ...news, saved: true}" class="ml-auto" />
          </div>
          <div class="pr-12">
            ...
            <p class="text-xs text-error italic mt-2">Read before : {{ formatDate(news.readLater[0].selected_date) }}</p>
          </div>
        </div>
      </a>
    </div>
  ...
  <!-- Modal mark as read -->
  <Teleport v-if="authStore.action === 'read-management'" to="#modal-container">
    <input type="checkbox" id="read-management" class="modal-toggle" @change="authStore.closeModal(!$event.target.checked)" />
    <label for="read-management" class="modal modal-open cursor-pointer">
      <label class="modal-box relative" for="">
        <label for="read-management" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
        <h3 class="text-lg font-bold">Read status</h3>
        <form @submit.prevent="removeSavedRss(newsRead)" class="flex flex-col gap-4">
          <p class="py-4">Have you finished reading the {{ newsRead.author }} "<span class="italic underline">{{ newsRead.title }}</span>" article?</p>
          <p>If the answer is yes, it will be removed from the Read Later board. If the answer is no, it will not be deleted.</p>
          <div class="flex gap-2 w-full justify-end">
            <label for="read-management" class="btn btn-error btn-outline">No</label>
            <button type="submit" class="btn btn-success hover:scale-[1.1]">Yes</button>
          </div>
        </form>
      </label>
    </label>
...
<script setup>
import { defineAsyncComponent, computed, ref, onMounted, inject, watch, toRefs } from 'vue'
...
const sortField = ref('selected_date')
const isDropdownOpen = ref(false)
const newsRead = ref(null)

const authStore = inject('authStore')
const savedNewsList = ref([])

const { rssIds } = toRefs(authStore)
watch(rssIds, (n, o) => {
  if (n != null) loadSavedNews()
})
onMounted(async () => {
  rssIds.value = null
...
const loadSavedNews = async () => {
  if (rssIds.value) {
    const { data: rssData, error: rssError } = await supabase
      .from('rss')
      .select('*, readLater(*)')
      .in('link', rssIds.value)
      .eq('readLater.user_id', authStore.user.userId)

    if (!rssError) savedNewsList.value = rssData
  }
}

const getStyle = (news) => {
  const currentDate = new Date()
  const selectedDate = new Date(news.readLater[0].selected_date)
  const diffTime = selectedDate - currentDate
  const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24))

  if (diffDays < 0) {
    return 'border border-error bg-danger'
  } else if (diffDays <= 1) {
...
const showReadManagement = (news) => {
  newsRead.value = news
  authStore.action = 'read-management'
}

const removeSavedRss = async (news) => {
  ...
  if (!error) {
    savedNewsList.value = savedNewsList.value.filter(n => n.id != news.id)
  }
  authStore.action = null
}

const savedNewsListSorted = computed(() => {
  const sortedList = savedNewsList.value.sort((a, b) => {
    const aValue = new Date(a.readLater[0][sortField.value]).getTime()
    const bValue = new Date(b.readLater[0][sortField.value]).getTime()
